...
import { generateQuiz, submitQuiz, getProgress, getQuizHistory, getQuizDetail, deleteQuizHistory } from '../service/Api';
import { Target, Loader2, ArrowRight, RotateCcw, BarChart3, Trophy, CheckCircle, XCircle, Search, Sparkles, Zap, BookOpen, Code, Globe, Database, Shield, Cpu, Gamepad2, Clock, ChevronLeft, ChevronRight, Eye, LogOut, Trash2 } from 'lucide-react';
...
const QuizPage = () => {
  ...
  useEffect(() => {
    ...
    const topicQuizzes = (progressData.quiz_history || []).filter(q => q.topic?.toLowerCase() === progressTopic);
  ...
  const handleViewDetail = async (quizId) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await generateQuiz(courseTitle || topic, level, numQuestions);
  ...
  const handleNext = async () => {
    if (currentQ < questions.length - 1) {
      ...
      setLoading(true);
      try {
        setQuizResult(await submitQuiz(courseTitle || topic, level, answers));
        setStep('results');
  ...
  const handleQuit = () => {
    ...
    setStep('config'); setQuestions([]); setAnswers([]); setCurrentQ(0);
    setSelectedAnswer(null); setShowResult(false); setQuizResult(null); setError('');
  };
  ...
  return (
    <>
        ...
        <div className="relative pt-20 pb-12 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
          ...
          {step === 'config' && !loading && (
            <div className="animate-fade-in-up">
              ...
              <div className="flex items-center justify-center gap-2 mb-8">
                ...
                >
                  <Sparkles className="w-4 h-4" /> Nouveau Quiz
                </button>
                <button
                ...
                >
                  ...
                  {quizHistory.length > 0 && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${showHistory ? 'bg-white/20 text-white' : 'bg-purple-500/20 text-purple-400'}`}>
                      {quizHistory.length}
                    </span>
                  )}
                </button>
                ...
                <div className="animate-fade-in-up">
                  {quizHistory.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 text-center">
                      ...
                      <p className="text-gray-400 font-medium mb-1">Aucun quiz enregistré</p>
                      <p className="text-gray-600 text-sm">Passez votre premier quiz pour voir l'historique ici.</p>
                    </div>
                  ) : (
                      ...
                      <div className="p-6">
                        ...
                        <div className="space-y-2 max-h-[60vh] overflow-y-auto custom-scroll pr-1">
                          {quizHistory.map((q) => {
                            ...
                            return (
                              <div key={q.id} className="group flex items-center justify-between py-3 px-4 rounded-xl bg-gray-800/30 hover:bg-gray-800/60 transition-colors cursor-pointer border border-transparent hover:border-indigo-500/20"
                                ...
                                <div className="flex items-center gap-3 flex-shrink-0">
                                  ...
                                  <Eye className="w-4 h-4 text-gray-600 group-hover:text-indigo-400 transition-colors flex-shrink-0" />
                                </div>
                              </div>
                            );
                          })}
                    ...
                    <div className="p-8">
                      ...
                      {!showTopicList ? (
                        <button
                          ...
                          className="w-full mb-6 flex items-center justify-between px-4 py-3 bg-gray-800/40 border border-gray-700/40 rounded-xl text-sm text-gray-400 hover:border-indigo-500/40 hover:text-gray-300 transition-all"
                        >
                                ...
                                <div key={cat.name}>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {cat.topics.map(t => (
                                      <button
                                        key={t}
                                        onClick={() => { setTopic(t); setSearchQuery(''); setShowTopicList(false); }}
                            ...
                            {filteredCategories.length === 0 && (
                              <div className="flex flex-col items-center gap-3 py-6">
                                ...
                                <button
                                  ...
                                  className="px-4 py-2 bg-violet-500/15 border border-violet-500/40 text-violet-300 rounded-lg text-xs font-medium hover:bg-violet-500/25 transition-all flex items-center gap-2"
                                >
                            ...
                            {levelLoading ? (
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-xs px-2 py-0.5 rounded-full border ${levelBg}`}>
                                  {(() => {
                                    const progressTopic = (courseTitle || topic).toLowerCase();
                                    const count = (progressData?.quiz_history || []).filter(q => q.topic?.toLowerCase() === progressTopic).length;
                        ...
                        <div className="grid grid-cols-4 gap-3">
                          {[
                            ...
                            <button
                              ...
                              className={`py-3 rounded-xl text-sm font-semibold border transition-all duration-200 flex flex-col items-center gap-1 ${numQuestions === n
          ...
          {loading && step === 'config' && (
            <div className="flex flex-col items-center justify-center py-32 animate-fade-in">
              ...
              <p className="text-gray-500 text-sm">L'IA prépare un quiz sur <span className="text-indigo-400">{topic}</span></p>
            </div>
          )}
          ...
          {step === 'quiz' && questions[currentQ] && (
            <div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
                  <div className="bg-gray-900 border border-gray-700/60 rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl">
                    <div className="flex flex-col items-center text-center gap-4">
                      ...
                      <h3 className="text-white font-bold text-lg">Quitter le quiz ?</h3>
                      <p className="text-gray-400 text-sm">Ta progression sur ce quiz sera perdue. Es-tu sûr de vouloir quitter ?</p>
                      <div className="flex gap-3 w-full mt-2">
                        <button
                          onClick={() => setShowQuitModal(false)}
                          className="flex-1 py-3 rounded-xl bg-gray-800 border border-gray-700/50 text-gray-300 font-medium hover:bg-gray-700 transition-all"
                        >
                          Continuer
                        </button>
                        <button
                          onClick={handleQuit}
                          className="flex-1 py-3 rounded-xl bg-rose-500/15 border border-rose-500/40 text-rose-400 font-medium hover:bg-rose-500/25 transition-all"
                        >
                          Quitter
                        </button>
                      </div>
              ...
              <div className="flex items-center justify-center gap-3 mb-6 animate-fade-in relative">
                ...
                <button
                  ...
                  className="absolute right-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-800/60 border border-gray-700/40 text-gray-500 hover:text-rose-400 hover:border-rose-500/40 hover:bg-rose-500/10 text-xs font-medium transition-all"
                >
      ...
      <AiAssistant
        context="quiz"
        subject={topic || 'Quiz'}
        extra={questions?.[currentQ]?.question || ''}
      />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-gray-900 border border-gray-700/60 rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl">
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <p className="text-gray-400 text-sm">Tous tes quiz enregistrés seront supprimés définitivement. Cette action est irréversible.</p>
              <div className="flex gap-3 w-full mt-2">
                <button
      ...
      {/* ── Detail Modal (portal-level, covers full screen) ── */}
      {selectedQuiz && (
        <div
          className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 backdrop-blur-sm animate-fade-in p-4 pt-20"
          onClick={handleCloseDetail}
        >
          <div
            className="w-full max-w-2xl bg-gray-900 border border-gray-700/60 rounded-2xl overflow-hidden shadow-2xl max-h-[calc(100vh-96px)] flex flex-col"
                ...
                <>
                  <div className="flex items-center justify-between mb-5 flex-shrink-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-base font-bold text-white">{quizDetail.topic}</h3>
                        <p className="text-xs text-gray-500 capitalize">{quizDetail.level} • {quizDetail.taken_at ? new Date(quizDetail.taken_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      {(() => {
                        const pct2 = quizDetail.total_questions ? Math.round((quizDetail.score / quizDetail.total_questions) * 100) : 0;
                        const c = pct2 >= 80 ? 'text-emerald-400' : pct2 >= 50 ? 'text-amber-400' : 'text-rose-400';
                        return <span className={`text-xl font-bold ${c}`}>{quizDetail.score}/{quizDetail.total_questions}</span>;
                      })()}
                      <p className="text-xs text-gray-500">Score</p>
                    </div>
                  </div>
                  <div className="space-y-3 overflow-y-auto custom-scroll pr-1 flex-1 min-h-0">
                    {quizDetail.details && quizDetail.details.length > 0 ? (
                      quizDetail.details.map((d, i) => (
                        <div key={i} className={`p-4 rounded-xl border ${d.is_correct ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
                          <div className="flex items-start gap-3">
                            <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${d.is_correct ? 'bg-emerald-500/15' : 'bg-red-500/15'}`}>
                              {d.is_correct ? <CheckCircle className="w-3.5 h-3.5 text-emerald-400" /> : <XCircle className="w-3.5 h-3.5 text-red-400" />}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-white mb-2 leading-snug"><span className="text-gray-500 mr-1">{i + 1}.</span>{d.question}</p>
                              <p className="text-xs text-gray-400">
                                Votre réponse : <span className={d.is_correct ? 'text-emerald-400 font-medium' : 'text-red-400 font-medium'}>{d.user_answer}</span>
                              </p>
                              {!d.is_correct && (
                                <p className="text-xs text-gray-400 mt-0.5">
                                  Bonne réponse : <span className="text-emerald-400 font-medium">{d.correct_answer}</span>
                                </p>
                              )}
                              {d.explanation && (
                                <p className="text-xs text-indigo-300/70 mt-2 italic leading-relaxed border-l-2 border-indigo-500/30 pl-2">{d.explanation}</p>
                              )}
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="text-center py-10">
                        <p className="text-gray-500 text-sm">Les détails de ce quiz ne sont pas disponibles.</p>
                        <p className="text-gray-600 text-xs mt-1">Seuls les quiz passés après la mise à jour sont enregistrés avec détails.</p>
                      </div>
                    )}
                  </div>
                </>
